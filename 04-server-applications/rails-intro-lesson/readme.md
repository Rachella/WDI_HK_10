...
* Rails 4.0 (Jun 2013) - Strong Parameters, Turbolinks, Cache Digest (aka Russian Doll Caching)
...
The goal of the next few minutes is to show the power that Rails gives us – it's actually possible to create a website with a lot of the functionality – forms, links, database, and MVC structure – in less than 5 minutes. We will not detail each step for this app, but we will create a dynamic website in 5 mins by typing the following commands.
...
In RESTful Web API, we use the HTTP method (GET, POST, PUT, DELETE) to interact with resources. In Rails, we use Routes (combined with the HTTP verb/method) to specify the action we should perform on a resource. We will discuss how to do this below and in the Rails Routes lesson.

#### Rails Routing vs. Hapi Routing
...
When we used Hapi, we were managing the routes and the code executed for a specific route in the same place. Below is a short example of how we define the `/` route (root path) in Hapi.
...
We've already defined what a RESTful resource is and we have also talked about how to create a Rails controller, let's see how we can combine the two concepts together and build RESTful controllers.

In Rails, a RESTFul resource has 7 methods. We will discuss this in more details in another lesson so right now we will just show you a sneak peek.

* Index   - GET method. Show all items.
* Show    - GET metjod. Show one item.
* New     - GET method. Display a HTML form for creating new item.
* Create  - POST method. Create a new item.
* Edit    - GET method. Display a HTML form for updating an existing item.
* Update  - PUT/PATCH method. Update an existing item.
* Delete  - DELETE method. Delete/Destroy an existing item.

Rails has a generator called `scaffold` that will create the whole MVC structure (all 7 methods) for a resource, let's say that inside the cookbook app, we want the `Recipe` resource to have a title and a content field, we would type:
